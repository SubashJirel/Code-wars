...
function dirReduc(arr) {
  ...
  for (let i = 0; i < newArr.length; i++) {
    if (
      ...
      // Skip the next direction as it's opposite to the current one
      i++;
    } else {
      simplifiedArr.push(newArr[i]);
    }
  }

  // Check if there are still further simplifications possible
  if (simplifiedArr.length < newArr.length) {
    return dirReduc(simplifiedArr);
  } else {
